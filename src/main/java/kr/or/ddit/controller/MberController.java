...
public class MberController {
	...
	public String getHshldList(Model model,
			...
			@RequestParam(value = "keyword", required = false, defaultValue = "") String keyword, //검색을 할 경우 사용
			@RequestParam(value="size", required=false, defaultValue="20") int size //한페이지에 보여줄 글 수 20개
			) {
		
	    Map<String, Object> map = new HashMap<>();
	    map.put("currentPage", currentPage);	// 현재페이지
	    map.put("size", size);      //페이징
	    map.put("keyword", keyword); // 검색을 할 경우 사용
